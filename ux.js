...
function renderUX(){
  ...
  [["num",daily,SERIES_LEN],["let",ldaily,LET_ROUNDS]].forEach(([prefix,state,total]) => {
    ...
    $(prefix + "Home").textContent = t.universe;
  });
...
function launchFromHub(nextGame, nextMode){
  // Une session Blitz possède son plateau : la reprendre avant d'en ouvrir une autre.
  if (blitz.running || lblitz.running){ setGame(lblitz.running ? "let" : "num"); startBlitzScreen(); return; }
  setGame(nextGame);
  if (nextMode === "daily") startDaily(); else if (nextMode === "blitz") startBlitzScreen(); else go("home");
}
...
for (const prefix of ["num","let"]){
  ...
  $(prefix + "Home").addEventListener("click",()=>launchFromHub(prefix,"home"));
}
