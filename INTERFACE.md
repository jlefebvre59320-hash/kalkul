...
## Bip en relief et variété des tirages — v21

Bip utilise un relief vectoriel (dégradé radial, lumière de bord, reflet supérieur, éclats dans les yeux et ombre de contact), sans moteur WebGL ni modèle 3D. Le corps et le visage flottent ensemble ; les animations de réaction, tenues, couleurs et accessoires restent compatibles. Les réglages Calme et réduction des mouvements sont respectés. Vingt nouvelles répliques complètent les quatre caractères ; les répliques d’un groupe sont mélangées et épuisées avant répétition.

Le moteur pur `variety.js` indexe 134 406 combinaisons distinctes de six nombres, avec zéro à trois grands nombres parmi 25/50/75/100 et des nombres distincts parmi 1 à 20. Une permutation complète évite les répétitions dans son cycle. Les tirages aléatoires évitent 200 combinaisons récentes en mémoire ; les signatures complètes restent protégées par l’historique existant de 3 000 puzzles. La complexité des candidats compte maintenant les opérations réellement effectuées dans les deux branches de calcul. Les cibles retenues sont contrôlées par le solveur. La difficulté reste une estimation, pas une preuve du nombre minimal d’opérations.

Les graines de mots FR/EN de 6 à 9 lettres sont extraites des dictionnaires existants, dédoublonnées par anagramme, puis échantillonnées de manière déterministe (jusqu’à 4 000 par longueur/langue). Le tirage ajoute des lettres pondérées avant mélange. Un mot d’au moins six lettres est ainsi garanti. L’historique local des 240 derniers tirages de Blitz évite les mêmes anagrammes et pénalise les tirages très proches des douze derniers ; les recherches sont bornées à 80 essais. Ce mécanisme réduit la redondance sans promettre une absence éternelle de répétitions.

Un index d’anagrammes énumère au plus 511 sous-ensembles des neuf lettres pour trouver toutes les solutions du dictionnaire au lieu d’en rescanner tous les mots. Après un quotidien Lettra terminé, dix panneaux permettent d’explorer ces solutions. Aucune réponse supplémentaire n’est dévoilée en partie.

Migration : les quotidiens antérieurs au 23 septembre 2026 gardent les anciens générateurs, dont le calcul des difficultés ; les nouveaux quotidiens restent déterministes et indépendants de l’historique personnel. Blitz et défi surprise utilisent le nouvel espace immédiatement. Publier word-seeds.js, variety.js, discoveries.js et bip-volume.css ; cache v21.

Validation : `node tests/generation.cjs` contrôle l’intégralité de l’espace numérique, 500 tirages FR et 500 EN sans répétition d’anagramme dans ces séries, validité de toutes les graines, comparaison exacte du solveur avec un parcours exhaustif du dictionnaire sur 40 tirages, résolution de 30 cibles quotidiennes, déterminisme et conservation des quotidiens du 22 septembre. Vérification navigateur à 390×844 : studio, réaction, aperçu Mangue, lancement du Blitz Lettres, validation de SALUER et progression au tirage suivant, console sans erreur. Syntaxe des scripts et présence de toutes les ressources hors ligne vérifiées. Pas de validation sur appareil physique ni de garantie absolue de difficulté ou de nouveauté infinie.
